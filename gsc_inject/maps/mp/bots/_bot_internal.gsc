...
initAStar( goal )
{
	// iw3mp recomp: throttle + DESYNC A* so many bots don't run it in the same frame. AStarSearch
	// creates a spawnstruct/variables per node; with 11 bots all pathing at once (spawn, or when a
	// death clears threats) the concurrent GSC variable/endon usage blows the engine limit -> freeze.
	// Re-path at most every ~0.6-1.1s per bot (jittered so bots desync), reusing the cached path.
	if ( isdefined( self.bot.astar_time ) && isdefined( self.bot.astar ) && self.bot.astar.size > 0 &&
	     ( gettime() - self.bot.astar_time ) < self.bot.astar_cooldown )
	{
		return self.bot.astar.size - 1;
	}
	self.bot.astar_time = gettime();
	self.bot.astar_cooldown = 250 + randomint( 350 );   // shorter (fewer bots) -> snappier re-path, less stuck
