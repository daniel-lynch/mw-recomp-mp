...
// Slug a gamertag into a filesystem-safe profile id (lowercase alnum + dashes). The id is what the
// content dir / profile selection key on, so it must be stable and path-safe.
...
// Derive a deterministic XUID from a profile id (FNV-1a). The 0xB13E prefix keeps the high mask
// bits (0x00C0...0000) clear so the game's "permissions" XUID check passes.
